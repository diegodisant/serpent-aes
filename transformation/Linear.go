...
// Linear transformation used to represent a list of 128 lists
// one for each output bit from 0 to 127 with position of input bits

var LinearTransformationTable []t.TransitionTable = []t.TransitionTable{
...
var LinearInverseTransformationTable []t.TransitionTable = []t.TransitionTable{
	[]int{53, 55, 72},
	[]int{1, 5, 20, 90},
	[]int{15, 102},
	[]int{3, 31, 90},
...
func LinearTransformation(input t.Bitstring) t.Bitstring {
	if len(input) != 128 {
		fmt.Println("Input bitstring doesn't have a 128 fixed size")
	}

	var tableLength int = len(LinearTransformationTable)
	var transformedOutput t.Bitstring
	var outputBit t.Bitstring
	var temporaryBitstring t.Bitstring

	for tableIndex := 0; tableIndex < tableLength; tableIndex++ {
		outputBit = "0"

		for _, transitionTableColumnIndex := range LinearTransformationTable[tableIndex] {
			temporaryBitstring = t.Bitstring(input[transitionTableColumnIndex])
			outputBit = outputBit.BinaryXor(temporaryBitstring)
		}

		transformedOutput = transformedOutput + t.Bitstring(outputBit)
	}

	return transformedOutput
}

func LinearInverseTransformation(output t.Bitstring) t.Bitstring {
	if len(output) != 128 {
		fmt.Println("Output bitstring doesn't have a 128 bit fixed size")
	}

	var tableLength int = len(LinearInverseTransformationTable)
	var transformedInput t.Bitstring
	var inputBit t.Bitstring
	var temporaryBitstring t.Bitstring

	for tableIndex := 0; tableIndex < tableLength; tableIndex++ {
		inputBit = "0"

		for _, transitionTableColumnIndex := range LinearInverseTransformationTable[tableIndex] {
			temporaryBitstring = t.Bitstring(output[transitionTableColumnIndex])
			inputBit = inputBit.BinaryXor(temporaryBitstring)
		}

		transformedInput = transformedInput + t.Bitstring(inputBit)
	}

	return transformedInput
}

func LinearTransformationBitSlice(s t.BitSlice) t.BitSlice {
	s[0] = s[0].RotateLeft(13)
	s[2] = s[2].RotateLeft(3)
	s[1] = s[1].BitSliceXor(t.BitSlice{s[1], s[0], s[2]})
	s[3] = s[3].BitSliceXor(t.BitSlice{s[3], s[2], s[0].ShiftLeft(3)})
	s[1] = s[1].RotateLeft(1)
	s[3] = s[3].RotateLeft(7)
	s[0] = s[0].BitSliceXor(t.BitSlice{s[0], s[1], s[3]})
	s[2] = s[2].BitSliceXor(t.BitSlice{s[2], s[3], s[1].ShiftLeft(7)})
	s[0] = s[0].RotateLeft(5)
	s[2] = s[2].RotateLeft(22)

	return s
}

func LinearInverseTransformationBitSlice(s t.BitSlice) t.BitSlice {
	s[2] = s[2].RotateRight(22)
	s[0] = s[0].RotateRight(5)
	s[2] = s[2].BitSliceXor(t.BitSlice{s[2], s[3], s[1].ShiftLeft(7)})
	s[0] = s[0].BitSliceXor(t.BitSlice{s[0], s[1], s[3]})
	s[3] = s[3].RotateRight(7)
	s[1] = s[1].RotateRight(1)
	s[3] = s[3].BitSliceXor(t.BitSlice{s[3], s[2], s[0].ShiftLeft(3)})
	s[1] = s[1].BitSliceXor(t.BitSlice{s[1], s[0], s[2]})
	s[2] = s[2].RotateRight(3)
	s[0] = s[0].RotateRight(13)
